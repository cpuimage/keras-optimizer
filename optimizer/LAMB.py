...
class LAMB(optimizer.Optimizer):
    r"""Optimizer that implements the Layer-wise Adaptive Moments (LAMB).
    ...
    def update_step(self, gradient, variable):
        ...
        if self.amsgrad:
            ...
            v = v_hat
        var_t = (m * alpha) / (tf.sqrt(v) + self.epsilon)
        ...
        if var_t.shape.ndims != 0:
            ...
            ratio = tf.where(tf.greater(w_norm, 0), tf.where(
                tf.greater(g_norm, 0), lr * (w_norm / g_norm), lr), lr)
        else:
            ratio = lr
        variable.assign_sub(ratio * var_t)
