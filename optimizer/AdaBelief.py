...
class AdaBelief(optimizer.Optimizer):
    r"""Optimizer that implements the AdaBelief algorithm.
    ...
    def build(self, var_list):
        ...
        Args:
          var_list: list of model variables to build AdaBelief variables on.
        """
    ...
    def update_step(self, gradient, variable):
        ...
        if isinstance(gradient, tf.IndexedSlices):
            ...
            v.scatter_add(
                tf.IndexedSlices(
                    tf.square(m - gradient.values) * (1 - self.beta_2) + self.epsilon,
                    gradient.indices,
                )
            )
        else:
            ...
            v.assign_add((tf.square(m - gradient) - v) * (1 - self.beta_2) + self.epsilon)
        if self.amsgrad:
...
AdaBelief.__doc__ = AdaBelief.__doc__.replace(
