"""AdaMomentum optimizer implementation."""
...
class AdaMomentum(optimizer.Optimizer):
    r"""Optimizer that implements the AdaMomentum algorithm.
    ...
    def build(self, var_list):
        ...
        Args:
          var_list: list of model variables to build AdaMomentum variables on.
        """
    ...
    def update_step(self, gradient, variable):
        ...
        if isinstance(gradient, tf.IndexedSlices):
            ...
            v.scatter_add(
                tf.IndexedSlices(
                    tf.square(m) * (1 - self.beta_2) + self.epsilon,
                    gradient.indices,
                )
            )
        else:
            ...
            v.assign_add((tf.square(m) - v) * (1 - self.beta_2) + self.epsilon)
        if self.amsgrad:
            v_hat = self._velocity_hats[index]
            v_hat.assign(tf.maximum(v_hat, v))
            v = v_hat
        variable.assign_sub((m * alpha) / tf.sqrt(v))
...
AdaMomentum.__doc__ = AdaMomentum.__doc__.replace(
